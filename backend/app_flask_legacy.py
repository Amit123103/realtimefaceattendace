...
admin_auth = AdminAuth(multi_admin)
...
@app.route('/api/generate-student-qr', methods=['POST'])
def generate_student_qr():
    """Generate QR code for a student (public endpoint)"""
    try:
        data = request.json
        registration_no = data.get('registration_no')
        ...
        if not student:
            return jsonify({
                'success': False,
                'message': 'Student not found in database. Please register first.'
            }), 404
            
        # Generate QR
        img_io = qr_handler.generate_qr_bytes(registration_no, student['Name'])
        ...
        print(f"Error in generate_student_qr: {e}")
        return jsonify({
            'success': False,
            'message': f'Server error: {str(e)}'
        }), 500
...
def admin_login():
    ...
    try:
        ...
        if login_type == 'password':
            ...
            # Verify face
            success, username, confidence, message = face_engine.recognize_face(image_array, category='admin')
            
            if success:
                # We need to verify if this user is actually an ACTIVE admin in Excel
                # username here comes from the trained model (label map)
                is_valid_admin, admin_data = multi_admin.get_admin(username)
                
                if is_valid_admin and admin_data['Status'] == 'active':
                    session_token = admin_auth.create_session(username)
                    return jsonify({
                        'success': True,
...
def register_admin_face():
    ...
    try:
        ...
        # Register admin face
        # Save image to admins folder
        admins_dir = IMAGES_FOLDER / 'admins'
        admins_dir.mkdir(parents=True, exist_ok=True)
        
        # Sanitize username for filename
        from werkzeug.utils import secure_filename
        safe_username = secure_filename(username)
        image_path = admins_dir / f"{safe_username}.jpg"
        
        # Save image
        img = Image.fromarray(image_array)
        img.save(image_path)
        
        # Register admin face path FIRST so training can find it
        # Save image to admins folder (already done above)
        
        # Update admin record in Excel with image path
        admin_auth.register_admin_face(username, str(image_path))
        
        # Train admin model (now using Excel as source of truth)
        face_engine.train_model(category='admin')
...
def admin_face_status():
    ...
    try:
        ...
        has_face = admin_auth.has_face_registered(username)
...
@app.route('/api/admin/manage/delete', methods=['POST'])
def delete_admin_account():
    ...
    try:
        ...
        valid, role = admin_auth.verify_session(session_token)
        if not valid:
            return jsonify({'success': False, 'message': 'Invalid session'}), 401
            
        # Only super_admin can delete
        if role != 'super_admin':
            # Check if multi_admin supports check_permission, otherwise fallback to role check
            if not multi_admin_handler.check_permission(admin_auth.get_username(session_token), 'delete'):
        ...
        success, message = multi_admin_handler.delete_admin(username)
