...
class ExcelHandler:
    ...
    def add_student(self, name, registration_no, image_path):
        ...
        try:
            ...
            return False, "Error: Please close the Excel file (students.xlsx) and try again."
    ...
    def add_attendance(self, name, registration_no, image_path, mode='Face'):
        """Record attendance for a student"""
        try:
            # Read existing data from main attendance file
            ...
            # Store in dedicated QR file if mode is QR
            if mode == 'QR Code':
                qr_file = self.data_dir / 'qr_attendance.xlsx'
                if not qr_file.exists():
                    df_qr = pd.DataFrame(columns=['Name', 'Registration No', 'Date', 'Time'])
                    df_qr.to_excel(qr_file, index=False, engine='openpyxl')
                
                df_qr = pd.read_excel(qr_file, engine='openpyxl')
                qr_row = {
                    'Name': name,
                    'Registration No': registration_no,
                    'Date': today,
                ...
                df_qr = pd.concat([df_qr, pd.DataFrame([qr_row])], ignore_index=True)
                df_qr.to_excel(qr_file, index=False, engine='openpyxl')
